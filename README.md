...
- The group had to refactor and refine a big section of the website since the astro file was not consistent with the group's schema/markup/classes due to reasons. Additionally, because of this, we could not integrate the styling the group already has effectively. One of the refactoring includes renaming class names in the .css file into much more specific and scoped ones because we thought there might be a chance for it to affect other parts of the website when finally merging onto the big exhibit. Another major one was making the id names unique and turning them into classes to use the styles we already implemented and to allow both the demos to use the same layout. Last major one was fixing and removing the Astro "hydration" warnings.
